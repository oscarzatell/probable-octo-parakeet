...
async function ejectAsync(projectRoot, options) {
  ...
  if (!podsInstalled) {
    _log().default.nested(`- 🍫 When CocoaPods is installed, initialize the project workspace: ${_chalk().default.bold('cd ios && pod install')}`);
  }
...
async function getOrPromptForBundleIdentifier(projectRoot, defaultValue) {
  var _exp$ios;
...
async function getOrPromptForPackage(projectRoot, defaultValue) {
  var _exp$android;
