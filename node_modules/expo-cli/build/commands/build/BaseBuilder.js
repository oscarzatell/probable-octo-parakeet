...
class BaseBuilder {
  ...
  async checkForBuildInProgress() {
    ...
    if (process.env.EXPO_LEGACY_API === 'true') {
      buildStatus = await _xdl().Project.buildAsync(this.projectDir, {
        mode: 'status',
      ...
      buildStatus = await _xdl().Project.getBuildStatusAsync(this.projectDir, {
        platform: this.platform(),
        current: true,
        releaseChannel: this.options.releaseChannel,
        publicUrl: this.options.publicUrl,
...
Please see the docs (${_chalk().default.underline('https://docs.expo.io/versions/latest/guides/configuring-ota-updates/')}) and check if you can use them instead of building your app binaries again.`);
  ...
  async logBuildStatuses(buildStatus) {
    ...
    buildStatus.jobs.forEach((job, i) => {
      ...
      (0, _log().default)(`### ${i} | ${platform} | ${UrlUtils().constructBuildLogsUrl(job.id, username)} ###`);
...
`;
  ...
  async wait(buildId, {
    ...
    while (true) {
      ...
      if (process.env.EXPO_LEGACY_API === 'true') {
        res = await _xdl().Project.buildAsync(this.projectDir, {
          current: false,
          mode: 'status',
          ...(publicUrl ? {
            publicUrl
        ...
        res = await _xdl().Project.getBuildStatusAsync(this.projectDir, {
          current: false,
          ...(publicUrl ? {
            publicUrl
          } : {})
  ...
  async build(expIds) {
    ...
    if (process.env.EXPO_LEGACY_API === 'true') {
      ...
      result = await _xdl().Project.buildAsync(this.projectDir, opts);
    } else {
      let opts = {
        expIds,
        platform,
      ...
      result = await _xdl().Project.startBuildAsync(this.projectDir, opts);
    }
    ...
    if (buildId) {
      (0, _log().default)(`You can monitor the build at\n\n ${_chalk().default.underline(UrlUtils().constructBuildLogsUrl(buildId, username !== null && username !== void 0 ? username : undefined))}\n`);
    }
