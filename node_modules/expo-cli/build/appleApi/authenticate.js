...
function _getAppleIdFromParams({
  ...
  if (!(0, _lodash().some)([appleId, passedAppleIdPassword])) {
    return null;
  } // partial apple id params were set, assume user has intention of passing it in


  if (!(0, _lodash().every)([appleId, passedAppleIdPassword])) {
...
async function _promptForAppleId() {
  ...
  (0, _log().default)(wrap('Please enter your Apple Developer Program account credentials. ' + 'These credentials are needed to manage certificates, keys and provisioning profiles ' + `in your Apple Developer account.`)); // https://docs.expo.io/versions/latest/distribution/security/#apple-developer-account-credentials
