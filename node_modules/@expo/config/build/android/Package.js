...
function getCurrentPackageName(projectRoot) {
    ...
    let mainApplicationPath = glob_1.sync(path_1.default.join(packageRoot, '**', 'MainApplication.java'))[0];
    let packagePath = path_1.default.dirname(mainApplicationPath);
...
function renamePackageOnDisk(config, projectRoot) {
    ...
    // Set up our paths
    let packageRoot = getPackageRoot(projectRoot);
    let currentPackagePath = path_1.default.join(packageRoot, ...currentPackageName.split('.'));
    let newPackagePath = path_1.default.join(packageRoot, ...newPackageName.split('.'));
    // Create the new directory
    fs_extra_1.default.mkdirpSync(newPackagePath);
    // Move everything from the old directory over
    glob_1.sync(path_1.default.join(currentPackagePath, '**', '*')).forEach(filepath => {
    ...
    while (oldPathParts.length) {
        let pathToCheck = path_1.default.join(packageRoot, ...oldPathParts);
        try {
            let files = fs_extra_1.default.readdirSync(pathToCheck);
            if (files.length === 0) {
    ...
    const filesToUpdate = [
        ...glob_1.sync(path_1.default.join(newPackagePath, '**', '*')),
