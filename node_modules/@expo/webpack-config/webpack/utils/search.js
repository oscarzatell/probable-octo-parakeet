...
function getExpoBabelLoader(config) {
    var _a, _b;
    const { preLoaders = [], postLoaders = [], rules = [] } = config.module || {};
    const currentRules = getRulesAsItems(getRulesFromRules([...preLoaders, ...postLoaders, ...rules]));
...
function getLoadersFromRules(rules) {
    const loaders = rules.map(({ rule, index: ruleIndex }) => {
        ...
        return loaderToLoaderItemLoaderPart(rule.loaders || rule.loader || rule.use).map(loader => (Object.assign(Object.assign({}, loader), { rule,
            ruleIndex })));
    });
